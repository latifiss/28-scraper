...
const scrapeIvoryCoastCurrencies = async () => {
  try {
    const url = 'https://tradingeconomics.com/ivory-coast/currency';
    const headers = {
      'User-Agent':
    ...
    const currencies = ['USDXOF', 'EURXOF', 'GBPXOF'];
    const result = {};
    ...
    currencies.forEach((currency) => {
      ...
      if (row.length === 0) {
        console.warn(
          `Could not find data row for ${currency} on the Ivory Coast page.`,
        );
        return;
      }
    ...
    console.log('Scraped Ivory Coast currency data:', result);
    return result;
  } catch (error) {
    console.error('Error scraping Ivory Coast currency data:', error);
    throw error;
  }
};

module.exports = {
  scrapeIvoryCoastCurrencies,
};
