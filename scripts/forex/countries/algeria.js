...
const scrapeAlgeriaCurrencies = async () => {
  try {
    const url = 'https://tradingeconomics.com/algeria/currency';
    const headers = {
      'User-Agent':
    ...
    const currencies = ['USDDZD', 'EURDZD', 'GBPDZD'];
    const result = {};
    ...
    currencies.forEach((currency) => {
      ...
      if (row.length === 0) {
        console.warn(
          `Could not find data row for ${currency} on the Algeria page.`,
        );
        return;
      }
    ...
    console.log('Scraped Algeria currency data:', result);
    return result;
  } catch (error) {
    console.error('Error scraping Algeria currency data:', error);
    throw error;
  }
};

module.exports = {
  scrapeAlgeriaCurrencies,
};
