...
const scrapeKenyaCurrencies = async () => {
  try {
    const url = 'https://tradingeconomics.com/kenya/currency';
    const headers = {
      'User-Agent':
    ...
    const currencies = ['USDKES', 'EURKES', 'GBPKES'];
    const result = {};
    ...
    currencies.forEach((currency) => {
      ...
      if (row.length === 0) {
        console.warn(
          `Could not find data row for ${currency} on the Kenya page.`,
        );
        return;
      }
    ...
    console.log('Scraped Kenya currency data:', result);
    return result;
  } catch (error) {
    console.error('Error scraping Kenya currency data:', error);
    throw error;
  }
};

module.exports = {
  scrapeKenyaCurrencies,
};
