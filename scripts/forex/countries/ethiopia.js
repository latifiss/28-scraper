...
const scrapeEthiopiaCurrencies = async () => {
  try {
    const url = 'https://tradingeconomics.com/ethiopia/currency';
    const headers = {
      'User-Agent':
    ...
    const currencies = ['USDETB', 'EURETB', 'GBPETB'];
    const result = {};
    ...
    currencies.forEach((currency) => {
      ...
      if (row.length === 0) {
        console.warn(
          `Could not find data row for ${currency} on the Ethiopia page.`,
        );
        return;
      }
    ...
    console.log('Scraped Ethiopia currency data:', result);
    return result;
  } catch (error) {
    console.error('Error scraping Ethiopia currency data:', error);
    throw error;
  }
};

module.exports = {
  scrapeEthiopiaCurrencies,
};
