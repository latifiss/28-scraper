...
const scrapeAngolaCurrencies = async () => {
  try {
    const url = 'https://tradingeconomics.com/angola/currency';
    const headers = {
      'User-Agent':
    ...
    const currencies = ['USDAOA', 'EURAOA', 'GBPAOA'];
    const result = {};
    ...
    currencies.forEach((currency) => {
      ...
      if (row.length === 0) {
        console.warn(
          `Could not find data row for ${currency} on the Angola page.`,
        );
        return;
      }
    ...
    console.log('Scraped Angola currency data:', result);
    return result;
  } catch (error) {
    console.error('Error scraping Angola currency data:', error);
    throw error;
  }
};

module.exports = {
  scrapeAngolaCurrencies,
};
