...
const scrapeGhanaCurrencies = async () => {
  try {
    const url = 'https://tradingeconomics.com/ghana/currency';
    const headers = {
      'User-Agent':
    ...
    const currencies = ['USDGHS', 'EURGHS', 'GBPGHS'];
    const result = {};
    ...
    currencies.forEach((currency) => {
      ...
      if (row.length === 0) {
        console.warn(
          `Could not find data row for ${currency} on the Ghana page.`,
        );
        return;
      }
    ...
    console.log('Scraped Ghana currency data:', result);
    return result;
  } catch (error) {
    console.error('Error scraping Ghana currency data:', error);
    throw error;
  }
};

module.exports = {
  scrapeGhanaCurrencies,
};
