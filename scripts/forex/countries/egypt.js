...
const scrapeEgyptCurrencies = async () => {
  try {
    const url = 'https://tradingeconomics.com/egypt/currency';
    const headers = {
      'User-Agent':
    ...
    const currencies = ['USDEGP', 'EUREGP', 'GBPEGP'];
    const result = {};
    ...
    currencies.forEach((currency) => {
      ...
      if (row.length === 0) {
        console.warn(
          `Could not find data row for ${currency} on the Egypt page.`,
        );
        return;
      }
    ...
    console.log('Scraped Egypt currency data:', result);
    return result;
  } catch (error) {
    console.error('Error scraping Egypt currency data:', error);
    throw error;
  }
};

module.exports = {
  scrapeEgyptCurrencies,
};
