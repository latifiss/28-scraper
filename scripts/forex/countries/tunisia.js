...
const scrapeTunisiaCurrencies = async () => {
  try {
    const url = 'https://tradingeconomics.com/tunisia/currency';
    const headers = {
      'User-Agent':
    ...
    const currencies = ['USDTND', 'EURTND', 'GBPTND'];
    const result = {};
    ...
    currencies.forEach((currency) => {
      ...
      if (row.length === 0) {
        console.warn(
          `Could not find data row for ${currency} on the Tunisia page.`,
        );
        return;
      }
    ...
    console.log('Scraped Tunisia currency data:', result);
    return result;
  } catch (error) {
    console.error('Error scraping Tunisia currency data:', error);
    throw error;
  }
};

module.exports = {
  scrapeTunisiaCurrencies,
};
