...
const scrapeNigeriaCurrencies = async () => {
  try {
    const url = 'https://tradingeconomics.com/nigeria/currency';
    const headers = {
      'User-Agent':
    ...
    const currencies = ['USDNGN', 'EURNGN', 'GBPNGN'];
    const result = {};
    ...
    currencies.forEach((currency) => {
      ...
      if (row.length === 0) {
        console.warn(`Could not find data for ${currency}`);
        return;
      }
    ...
    console.log('Scraped Nigeria currency data:', result);
    return result;
  } catch (error) {
    console.error('Error scraping Nigeria currency data:', error);
    throw error;
  }
};

module.exports = {
  scrapeNigeriaCurrencies,
};
