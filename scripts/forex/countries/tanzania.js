...
const scrapeTanzaniaCurrencies = async () => {
  try {
    const url = 'https://tradingeconomics.com/tanzania/currency';
    const headers = {
      'User-Agent':
    ...
    const currencies = ['USDTZS', 'EURTZS', 'GBPTZS'];
    const result = {};
    ...
    currencies.forEach((currency) => {
      ...
      if (row.length === 0) {
        console.warn(
          `Could not find data row for ${currency} on the Tanzania page.`,
        );
        return;
      }
    ...
    console.log('Scraped Tanzania currency data:', result);
    return result;
  } catch (error) {
    console.error('Error scraping Tanzania currency data:', error);
    throw error;
  }
};

module.exports = {
  scrapeTanzaniaCurrencies,
};
