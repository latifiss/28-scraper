...
const scrapeMoroccoCurrencies = async () => {
  try {
    const url = 'https://tradingeconomics.com/morocco/currency';
    const headers = {
      'User-Agent':
    ...
    const currencies = ['USDMAD', 'EURMAD', 'GBPMAD'];
    const result = {};
    ...
    currencies.forEach((currency) => {
      ...
      if (row.length === 0) {
        console.warn(
          `Could not find data row for ${currency} on the Morocco page.`,
        );
        return;
      }
    ...
    console.log('Scraped Morocco currency data:', result);
    return result;
  } catch (error) {
    console.error('Error scraping Morocco currency data:', error);
    throw error;
  }
};

module.exports = {
  scrapeMoroccoCurrencies,
};
